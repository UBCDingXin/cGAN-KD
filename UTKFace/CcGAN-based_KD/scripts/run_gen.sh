...
NFAKE_PER_LABEL=5000
NUM_FAKE_LABELS=-1
filtering_threshold=0.7


echo "-------------------------------------------------------------------------------------------------"
echo "UTKFace; ${GAN_NAME}: No Subsampling, No filtering"
...
2>&1 | tee output_${GAN_NAME}_subsampling_False_filter_False_seed_${SEED}.txt

echo "-------------------------------------------------------------------------------------------------"
echo "UTKFace; ${GAN_NAME}: Subsampling, No filtering"
CUDA_VISIBLE_DEVICES=0,1 python3 generate_synthetic_data.py \
--root_path $ROOT_PATH --data_path $REAL_DATA_PATH --seed $SEED --num_workers $NCPU \
--gan_name $GAN_NAME \
--gan_dim_embed 128 --gan_embed_x2y_net_name ResNet34 \
...
--samp_filter_mae_percentile_threshold $filtering_threshold --unfiltered_fake_dataset_filename fake_UTKFace_SAGAN_hinge_niters_40000_subsampling_cDRE-F-SP+RS_hard_1e-20_FilterMAEPct_1.0_nfake_300000_seed_2020.h5 \
2>&1 | tee output_${GAN_NAME}_subsampling_True_filter_${filtering_threshold}_seed_${SEED}.txt
