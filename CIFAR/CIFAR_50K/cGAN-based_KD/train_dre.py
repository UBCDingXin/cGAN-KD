...
# training function
def train_dre(which_class, trainloader, dre_net, dre_precnn_net, netG, path_to_ckpt=None):
    ...
    # optimizer = torch.optim.Adam(dre_net.parameters(), lr = dre_lr_base, betas=(0, 0.999))


    if path_to_ckpt is not None and dre_resume_epoch>0:
        print("Loading ckpt to resume training dre_net >>>")
        ckpt_fullpath = path_to_ckpt + "/DRE_{}_class_{}_checkpoint_epoch_{}.pth".format(dre_net_name, which_class, dre_resume_epoch)
        checkpoint = torch.load(ckpt_fullpath)
        dre_net.load_state_dict(checkpoint['net_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

        #load d_loss and g_loss
        logfile_fullpath = path_to_ckpt + "/DRE_{}_class_{}_train_loss_epoch_{}.npz".format(dre_net_name, which_class, dre_resume_epoch)
    ...
    for epoch in range(dre_resume_epoch, dre_epochs):
        ...
        print("DRE+{}+lambda{}: [class {}] [epoch {}/{}] [train loss {}] [Time {}]".format(dre_net_name, dre_lambda, which_class, epoch+1, dre_epochs, train_loss/(batch_idx+1), timeit.default_timer()-start_time))

        avg_train_loss.append(train_loss/(batch_idx+1))

        # save checkpoint
        if path_to_ckpt is not None and ((epoch+1) % 200 == 0 or (epoch+1)==dre_epochs):
            ckpt_fullpath = path_to_ckpt + "/DRE_{}_class_{}_checkpoint_epoch_{}.pth".format(dre_net_name, which_class, epoch+1)
            torch.save({
                    'epoch': epoch,
            ...
            # save loss
            logfile_fullpath = path_to_ckpt + "/DRE_{}_class_{}_train_loss_epoch_{}.npz".format(dre_net_name, which_class, epoch+1)
