...
Baseline Knowledge Distillation: BLKD
...
hf.close()
assert np.max(images_train)>1 and np.min(labels_train)>=0
nreal = len(images_train) ##number of real data

## load fake data if needed
if args.fake_dataset_name != 'None':
    fake_dataset_path = os.path.join(args.root_path, 'output/fake_data/Tiny-ImageNet_{}_seed_{}.h5'.format(args.fake_dataset_name, args.seed))
...
trainloader_cnn = torch.utils.data.DataLoader(trainset_cnn, batch_size=args.batch_size_train, shuffle=True, num_workers=args.num_workers)

testset_cnn = IMGs_dataset(images_test, labels_test, transform=transform_cnn_test)
testloader_cnn = torch.utils.data.DataLoader(testset_cnn, batch_size=args.batch_size_test, shuffle=False, num_workers=args.num_workers)
...
print("\n -----------------------------------------------------------------------------------------")
print("\n Start training {} as student net with {} params >>>".format(args.student, num_parameters_student))

# Filename
filename_cnn_ckpt = save_models_folder + '/ckpt_student_{}_epoch_{}_transform_{}_lambda_{}_T_{}_seed_{}_with_teacher_{}_data_{}.pth'.format(args.student, args.epochs, args.transform, args.lambda_kd, args.T_kd, args.seed, args.teacher, train_dataset_name)
print('\n'+filename_cnn_ckpt)

path_to_ckpt_in_train = save_models_folder + '/ckpts_in_train_student_{}_transform_{}_lambda_{}_T_{}_seed_{}_with_teacher_{}_data_{}'.format(args.student, args.transform, args.lambda_kd, args.T_kd, args.seed, args.teacher, train_dataset_name)
os.makedirs(path_to_ckpt_in_train, exist_ok=True)

# training
if not os.path.isfile(filename_cnn_ckpt):
    print("\n Start training the {} >>>".format(args.student))
    net_student = train_cnn(net_student, args.student, trainloader_cnn, testloader_cnn, epochs=args.epochs, resume_epoch=args.resume_epoch, save_freq=save_freq, lr_base=args.lr_base, lr_decay_factor=args.lr_decay_factor, lr_decay_epochs=lr_decay_epochs, weight_decay=args.weight_decay, seed = args.seed, extract_feature=False, path_to_ckpt = path_to_ckpt_in_train, net_teacher=net_teacher, lambda_kd=args.lambda_kd, T_kd=args.T_kd)
...
''' Dump test results '''
test_results_logging_fullpath = args.root_path + '/output/Test_results_BLKD_student_{}_teacher_{}_seed_{}_data_{}.txt'.format(args.student, args.teacher, args.seed, train_dataset_name)
...
with open(test_results_logging_fullpath, 'a') as test_results_logging_file:
    ...
    test_results_logging_file.write("\n Teacher:{} ({} params); Student:{} ({} params); seed: {} \n".format(args.teacher, args.student, num_parameters_teacher, num_parameters_student, args.seed))
